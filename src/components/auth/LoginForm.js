 ...
 * Si el usuario ingresa exactamente "ADMIN" (case-insensitive),
 * se transforma automáticamente a "admin@hrcore.com" antes de
 * enviar al backend. Esto permite al Super Admin usar el alias
 * corto en lugar del correo completo.
 *
 * Cualquier otro valor se envía tal cual al backend (asumimos
...
function LoginFormInner() {
  ...
  const [info, setInfo] = useState(
    registeredParam
      ? 'Cuenta creada. Revisa tu correo si requieres confirmación, luego inicia sesión.'
      : resetParam
  ...
  const onForgot = async (e) => {
    ...
    try {
      ...
      setForgotStatus({
        state: 'success',
        message: data.message || 'Si el correo existe, recibirás un enlace.',
      });
    } catch {
      setForgotStatus({
        state: 'success',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-900/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl p-6">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div>
                <h3 className="text-lg font-bold text-ink-900">Restablecer contraseña</h3>
                <p className="text-sm text-slate-600 mt-1">
                  Te enviaremos un enlace seguro a tu correo.
                </p>
              </div>
              <button
                onClick={() => { setForgotOpen(false); setForgotStatus({ state: 'idle', message: '' }); }}
            ...
            <form onSubmit={onForgot} className="space-y-4">
              ...
              {forgotStatus.state === 'success' && (
                <p className="rounded-md bg-emerald-50 border border-emerald-200 p-3 text-sm text-emerald-800">
                  {forgotStatus.message}
                </p>
              )}
              <button
                type="submit"
                disabled={forgotStatus.state === 'loading'}
                className="inline-flex w-full justify-center rounded-md bg-brand-600 hover:bg-brand-700 px-4 py-2.5 text-sm font-semibold text-white transition disabled:opacity-60"
              >
